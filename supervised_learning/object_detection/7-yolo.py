...
class Yolo:
    ...
    def predict(self, folder_path):
        """Prépare la prédiction pour chaque image dans folder_path.

        Pour l'instant cette méthode :
        - ouvre le dossier via `load_images`
        - prétraite les images via `preprocess_images`
        - prépare et itère sur chaque image (boucle prête pour la suite)

        Retourne une liste de dictionnaires contenant :
        - 'image' : image originale (BGR)
        - 'path' : chemin du fichier image
        - 'pimage' : image prétraitée (normalisée et redimensionnée)
        - 'image_shape' : tuple (hauteur, largeur) de l'image originale
        """
        # Ouvre le dossier et charge les images
        ...
        for i, img in enumerate(images):
            # Extrait les prédictions spécifiques à l'image i
