...
def data_to_input_and_output(df):
    ...
    # Itère de 0 jusqu'à l'index max permettant une fenêtre complète + une cible
    for index in range(len(df) - WINDOW_SIZE):
        # Récupère une fenêtre de WINDOW_SIZE lignes à partir de l'index
        input_sample = df[feature_cols].iloc[index:index + WINDOW_SIZE].values

        # Prix à la dernière heure de la fenêtre (index 23 = 24ème heure, car index 0 = 1ère heure)
        ...
        output_data.append(change)  # Ajoute le changement correspondant (en USD)

    return np.array(input_data), np.array(output_data)

# Fait les appels pour créer les input et les output pour les données d'entrainement et celle de test
...
# Standardisation du train, puis standardisation de test grace aux mean et std de train
...
np.savez('normal/normalization_params.npz',
...
# Prend les 80 premiers % de train_input & train_output (entrée et sortie attendue)
train_ds = tf.data.Dataset.from_tensor_slices((train_input[:train_size], train_output[:train_size]))
# Mélange les données, crée des batch de 32, et précharge les données
# Shuffle évite au modèle d'apprendre l'ordre chronologique au lieu des vrais patterns
train_ds = train_ds.shuffle(10000).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)

# Même chose mais avec les 20% restants pour créer un dataset de validation
# Permet d'évaluer le modèle sur des données qu'il n'a pas vues pendant l'entraînement
val_ds = tf.data.Dataset.from_tensor_slices((train_input[train_size:], train_output[train_size:]))
...
# ========== MODÈLE ==========
def train_neural_network(train_ds, val_ds, epochs=EPOCHS, learning_rate=LEARNING_RATE):
    ...
    callbacks = [
        tf.keras.callbacks.ModelCheckpoint('model/best.keras', save_best_only=True, monitor='val_loss'),
        tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=EARLY_STOPPING, restore_best_weights=True)
    ]
...
# Utilisation du model entrainé sur toutes les données de bitstamp & coinbase (pas d'entrainement)
train_pred_norm = model.predict(train_input).flatten()  # Performance sur données d'entrainement
test_pred_norm = model.predict(test_input).flatten()  # Performance sur données de test
...
# Appel à reconstruct_prices, récupère les valeurs prédites et les valeurs réelles
train_pred_prices, train_real_prices = reconstruct_prices(train, train_pred, train_output_raw)
test_pred_prices, test_real_prices = reconstruct_prices(test, test_pred, test_output_raw)
