...
def dense_market(df, threshold=0.95):
    # Extrait le jour et la minute de chaque timestamp pour calculer la densité
    df_temp = df.copy()
    ...
    # Calcule le ratio de couverture (minutes présentes / 1440 minutes par jour)
...
def min_to_hours(df):
    ...
    return df.resample('1h', on='Timestamp').agg({
        'Close': 'last',
        'Volume_(BTC)': 'sum',
...
def preprocess(files: list):
    ...
    for file in files:
        ...
        df_new = df.copy()  # Copie le dataframe dans une nouvelle variable (pour ne pas modifier le fichier original)
        ...
        # Trouve la première journée ayant au moins 95% de points (1368/1440 minutes)
        ...
        # Rééchantillonne à la minute et remplit les valeurs manquantes par propagation avant (ffill)
        df_new = df_new.set_index('Timestamp').resample('1min').ffill().reset_index()
...
# appel la fonction preprocess qui démarre le préprocess des fichiers données par la consigne
preprocess(files)
